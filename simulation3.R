...
# -------------------- Scenario 3 --------------------#
# This code reproduces one replicate with n = 60. The dimensions are p1 = 100, p2 = 25, p3 = 1. The ranks for the first two modes are r1 = r2 = 2. The sparsity levels for the first two modes are s1 = s2 = 5. Please refer to Section 5.1 for more detailed information. 
...
data.gen <- model$data.gen # The function generating the simulated data in Scenario 3.
...
output <- sapply(seq_len(times), function(i){
  ...
  true.dist <- do.call(c, lapply(fit.results, "[[", 1)) # subspace estimation error
  s.list <- lapply(fit.results, "[[", 2) # sparsity levels
  error <- do.call(c, lapply(fit.results, "[[", 3)) # testing error
